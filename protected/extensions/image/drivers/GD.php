...
class Image_GD_Driver extends Image_Driver {
	...
	public function process($image, $actions, $dir, $file, $render = FALSE)
	{
		...
		if (empty($create) OR ! function_exists($create))
			throw new CException('image type not allowed 1');

		// Make sure the image type is supported for saving
		if (empty($save) OR ! function_exists($save)){
			throw new CException($file . ' image type not allowed, extension ' . $ext);
		}
	...
	public function resizeWithEffect($properties)
	{
		$padding = 2; //минимальная рамка в пикселях (по умолчанию = 5)

		// Get the current width and height
		$src_width = imagesx($this->tmp_image);
		$src_height = imagesy($this->tmp_image);

		$image_vars = getimagesize($this->image['file']);
		if (!$image_vars) {
			return false;
		}
		...
		if (($src_type != IMAGETYPE_JPEG) && ($src_type != IMAGETYPE_PNG) && ($src_type != IMAGETYPE_GIF)) {
			return false;
		}

		// создаем задний фон для нового изображения
		$img = imagecreatetruecolor($properties['width'], $properties['height']);
		$grey = imagecolorallocate($img, 177, 177, 177);
		if(!imagefill($img, 0, 0, $grey)){
			return false;
		}

		// копируем из исходного изображения в фон и растягиваем без соблюдения пропорций
//		$result = imagecopyresized($img, $this->tmp_image,  0,  0,  0,  0, $properties['width'], $properties['height'], $src_width, $src_height);
//		if (!$result) return false;
//
//		// применяем к фону размытие
//		for ($i= 0; $i<=60; $i++) {
//			$result = imagefilter($img, IMG_FILTER_SMOOTH, 6);
//			if (!$result) return false;
//		}
//
...
//		}
		...
		$ratio = min( ($properties['width']-2*$padding)/$src_width , ($properties['height']-2*$padding)/$src_height );
		...
		if ($status = imagecopyresampled($img, $this->tmp_image, round(($properties['width']-$new_width)/2), round(($properties['height']-$new_height)/2),  0,  0, $new_width, $new_height, $src_width, $src_height))
		{
			// Swap the new image for the old one
			imagedestroy($this->tmp_image);
			$this->tmp_image = $img;
