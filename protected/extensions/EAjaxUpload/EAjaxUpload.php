...
class EAjaxUpload extends CWidget {
    ...
    public function run() {
        ...
        $config = array(
            ...
            'multiple' => true,
            'messages'=>array(
                'typeError'=>"Загрузить можно только {extensions}.",
                'sizeError'=>"{file} слишком большой, максимальный размер {sizeLimit}.",
                'minSizeError'=>"{file} маленький, минимальный размер {minSizeLimit}.",
                'emptyError'=>"{file} пустой, пожалуйста выберите другой файл.",
                'onLeave'=>"Файлы загружаются, если вы уйдете сейчас загрузка будет отменена."
            ),
        );
